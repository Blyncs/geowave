...
yum --enablerepo=geowave search geowave-0.9.3.1-apache

# Install GeoWave Accumulo iterator on a host (probably a namenode)
yum --enablerepo=geowave install geowave-0.9.3.1-apache-accumulo

# Update
yum --enablerepo=geowave install geowave-0.9.3.1-apache-*
----
...
ex: /usr/local/geowave -> /usr/local/geowave-0.9.3.1-hdp2 but there could also be another /usr/local/geowave-0.9.3.1-cdh5 still
...
geowave-home - status is auto.
 link currently points to /usr/local/geowave-0.9.3.1-hdp2
/usr/local/geowave-0.9.3.1-hdp2 - priority 90
/usr/local/geowave-0.9.3.1-cdh5 - priority 89
Current `best' version is /usr/local/geowave-0.9.3.1-hdp2.
----
